class TennisMatchSimulator {
    ...
    async processSimulation(result) {
        ...
        if (this.isSkipped) {
            this.processSkippedSimulation(result);
            return;
        }

        for (let i = 0; i < result.setDetails.length; i++) {
            const set = result.setDetails[i];
            if (set) await this.processSet(set, i, result.setDetails);
        }

        this.displayWinner(result.winner);
    }

    processSkippedSimulation(result) {
        ...
        this.currentGames = { p1: lastSet.p1Score, p2: lastSet.p2Score };
        this.currentPoints = { p1: '0', p2: '0' };

        this.updateScoreDisplay();

        const matchSummary = result.setDetails.map(set =>
            `<div class="set-summary">Set ${set.setNumber}: ${this.player1Name} ${set.p1Score} - ${set.p2Score} ${this.player2Name}</div>`
        ).join('');

        this.elements.matchHistory.innerHTML = `<div class="match-summary">${matchSummary}</div>`;
        this.displayWinner(result.winner);
    }
    ...
    async processSet(set, setIndex, setDetails) {
        ...
        this.currentSetHistory += `<div class="set-result"><strong>Set ${set.setNumber} Final:</strong> ${this.player1Name} ${set.p1Score} - ${set.p2Score} ${this.player2Name}</div>`;
    ...
    async processGame(game) {
        ...
        this.updateScoreDisplay();
        this.currentSetHistory += `<div class="game-result">Game to ${game.winnerIsPlayer1 ? this.player1Name : this.player2Name}</div>`;
        this.isPlayer1Serving = !this.isPlayer1Serving;
        await this.delay(1000);
    }

    formatScore(p1Score, p2Score, isCurrent = false) {
        const p1Serve = this.isPlayer1Serving ? ' 🎾' : '';
        const p2Serve = !this.isPlayer1Serving ? ' 🎾' : '';
        return `<span>${p1Score}${isCurrent ? p1Serve : ''} - ${p2Score}${isCurrent ? p2Serve : ''}</span>`;
    }
    ...
    updateSetScores(setDetails) {
        this.elements.setScores.innerHTML = setDetails.map(set =>
            `<div class="set-score-item"><div class="d-flex justify-content-between align-items-center"><strong>Set ${set.setNumber}:</strong><span>${this.player1Name} ${set.p1Score} - ${set.p2Score} ${this.player2Name}</span></div></div>`
        ).join('');
    }
    ...
    async processGamePoints(points) {
        ...
        this.elements.currentGamePoints.innerHTML = points.map(point =>
            `<div class="point-item"><div class="d-flex justify-content-between align-items-center"><span>${this.player1Name}</span><span class="score">${point.p1Score} - ${point.p2Score}</span><span>${this.player2Name}</span></div></div>`
        ).join('');
        ...
        for (const point of points) {
            if (point) {
                ...
                this.updateScoreDisplay();
                this.currentSetHistory += `<div class="point-score">${this.player1Name} ${point.p1Score} - ${point.p2Score} ${this.player2Name}</div>`;
    ...
    displayWinner(winner) {
        if (winner) {
            this.elements.matchHistory.innerHTML += `<div class="winner-section mt-4"><h3>Match Complete</h3><h2>Winner: ${winner.name}</h2></div>`;
        }
    }

    handleError(error) {
        console.error('Simulation error:', error);
        this.elements.matchHistory.innerHTML = `<div class="alert alert-danger"><h4>Error</h4><p>${error.message || 'Simulation failed. Please try again.'}</p></div>`;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const isCompleted = sessionStorage.getItem(`match_${matchId}_completed`) === 'true';
    ...
    const createSimulator = (isSkip = false) => {
        const simulator = new TennisMatchSimulator(
            elements,
            simulateBtn.dataset.player1 || 'Player 1',
            simulateBtn.dataset.player2 || 'Player 2'
        );
        simulator.isSkipped = isSkip;
        return simulator;
    };

    const handleButtonClick = async (isSkip) => {
        simulateBtn.disabled = true;
        skipToResultBtn.disabled = true;

        const simulator = createSimulator(isSkip);
        await simulator.simulate(matchId);
        sessionStorage.setItem(`match_${matchId}_completed`, 'true');
    };

    skipToResultBtn.addEventListener('click', () => handleButtonClick(true));
    simulateBtn.addEventListener('click', () => handleButtonClick(false));
});